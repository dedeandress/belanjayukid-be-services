...
case class Staff(id: UUID = UUID.randomUUID(), userId: UUID, roleId: UUID, status: Boolean = true)

object Staff extends ((UUID, UUID, UUID, Boolean) => Staff) {
  ...
  class StaffTable(slickTag: SlickTag) extends SlickTable[Staff](slickTag, "staff") {
    ...
    def * = (id, userId, roleId, status).mapTo[Staff]
...
object StaffJsonProtocol extends DefaultJsonProtocol {
  ...
  implicit val staffJsonProtocolFormat: JsonFormat[Staff] = jsonFormat4(Staff)
}
